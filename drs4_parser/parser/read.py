...
# The DRS4 chip samples the input signal by continuously charging a ring of 1024
# capacitors, each with a slightly different effective width in time. These widths
# are measured during calibration and stored in the file header.
# When a trigger arrives, the chip stops at a particular capacitor — the trigger
# cell (tcell) — which is different for every event. To reconstruct the time axis,
# the width array is rotated so that tcell becomes index 0, then cumulatively
# summed to convert widths into absolute times, with t[0] defined as 0.
